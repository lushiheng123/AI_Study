...
const Navbar = ({ toggleSidebar, isSidebarOpen }) => {
  ...
  useEffect(() => {
    const fetchModels = async () => {
      try {
        const response = await axios.get("http://localhost:5051/ollama/model_list", {
          withCredentials: true, // 如果需要携带 cookies（如 JWT 认证）
        });

        if (response.data && Array.isArray(response.data.models)) {
          setModels(response.data.models);
          if (response.data.models.length > 0) setSelected(response.data.models[0]);
        } else {
